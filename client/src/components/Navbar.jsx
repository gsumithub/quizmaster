...
import { Menu, X, Trophy, LogOut, LayoutDashboard, Settings, BookOpen, Sun, Moon } from 'lucide-react';
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {user ? (
            <>
              ...
              {user && user.role === 'admin' && (
                <a
                  ...
                  className="flex items-center space-x-2 px-3 py-2 rounded-lg text-sm font-medium text-amber-400 hover:text-amber-300 hover:bg-amber-950/20 border border-amber-500/20 transition-all duration-200"
                >
            ...
            <div className="flex items-center space-x-3">
              <Link
                to="/login"
                className="px-4 py-2 rounded-xl text-sm font-semibold text-dark-200 hover:text-white transition-all duration-200"
              >
                Sign In
              </Link>
              <Link
                to="/register"
                className="bg-primary-600 hover:bg-primary-500 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-primary-600/20 hover:scale-[1.02] active:scale-100 transition-all duration-200"
              >
                Sign Up
              </Link>
            </div>
            ...
            {user ? (
              <>
                ...
                {user && user.role === 'admin' && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center space-x-2 px-4 py-3 rounded-lg text-base font-medium text-amber-400 hover:text-amber-350 hover:bg-amber-950/20 border border-amber-500/20 transition-all duration-200"
                  >
              ...
              <div className="grid grid-cols-2 gap-2 p-2">
                <Link
                  to="/login"
                  onClick={() => setIsOpen(false)}
                  className="px-4 py-3 rounded-xl text-center text-base font-semibold text-dark-200 hover:text-white hover:bg-dark-800 transition-all duration-200"
                >
                  Sign In
                </Link>
                <Link
                  to="/register"
                  onClick={() => setIsOpen(false)}
                  className="bg-primary-600 hover:bg-primary-500 text-white py-3 rounded-xl text-center text-base font-semibold shadow-lg shadow-primary-600/20 transition-all duration-200"
                >
                  Sign Up
                </Link>
              </div>
